...
if ( isset( $_GET[ 'entry' ] ) && is_numeric( $_GET[ 'entry' ] ) && isset( $_GET[ 'realm' ] ) && is_numeric( $_GET[ 'realm' ] ) && $_GET[ 'cl' ] <= '1200' ) {


	include( 'stream.php' );

	$information = mysqli_fetch_array( mysqli_query( $stream, "SELECT `region`, `short` FROM `000_ovw_realms` WHERE `id` = '" . $_GET[ 'realm' ] . "'" ) );

	$table = '' . $information[ 'region' ] . '_' . $_GET[ 'realm' ] . '';

	$char_info = mysqli_fetch_array( mysqli_query( $stream, "SELECT `name` FROM `" . $table . "` WHERE `id` = '" . $_GET[ 'entry' ] . "'" ) );

	$key = '';

	$character = $char_info[ 'name' ];
	$region = $information[ 'region' ];
	$realm = $information[ 'short' ];
	...
	echo '<meta http-equiv="refresh" content="0;url=http://apinfo.gerritalex.de/" />';
}
