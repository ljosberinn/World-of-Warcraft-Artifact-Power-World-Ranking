...
for ( $repeat = 1; $repeat <= 20; $repeat++ ) {
	...
	$character_fetcher = mysqli_fetch_array( mysqli_query( $stream, "SELECT `c`, `r`, `s`  FROM `000_import` AS r1 JOIN (SELECT (RAND() * (SELECT MAX(id) FROM `000_import`)) AS id) AS r2 WHERE r1.id >= r2.id ORDER BY r1.id ASC LIMIT 1;" ) );

	$character = $character_fetcher[ 'c' ];
	$region = $character_fetcher[ 'r' ];
	$realm = $character_fetcher[ 's' ];
	...
	if ( $data != '' ) {
		...
		if ( $data[ 'level' ] == '110' ) {
			...
			$realm_id = mysqli_fetch_array( mysqli_query( $stream, "SELECT `id` FRO `000_ovw_realms` WHERE `name` = '" . $realm . "'" ) );
			$table_name = '' . $region . '_' . $realm_id[ 'id' ] . '';
			// CHECK IF ALREADY EXISTS IN REALM TABLE
			$query = mysqli_fetch_array( mysqli_query( $stream, "SELECT `logout` FROM `" . $table_name . "` WHERE `name` = '" . $character . "'" ) );

			if ( $query[ 'logout' ] != $logout ) {
			...
			<table>
				...
				<tbody>
					<td class="' . $cell_class . '"><a href="http://armory.gerritalex.de/?r=' . $region . '&s=' . $character_fetcher[ 's' ] . '&c=' . $character . '">' . $character . '</a><br /><a class="region" onclick="left_table_region_filter(this.innerText);">' . $region . '</a> <a class="realm" onclick="left_table_realm_filter(US_\' + this.innerText + \');">' . $character_fetcher[ 's' ] . '</a></td>
				...
				if ( $data[ 'guild' ][ 'name' ] != '' ) {
					...
					$realm = mysqli_fetch_array( mysqli_query( $stream, "SELECT `short` FROM `000_ovw_realms` WHERE `name` = '" . $character_fetcher[ 's' ] . "'" ) );
					$realm = $realm[ 'short' ];
					...
					if ( $data != '' ) {
						...
						foreach ( $data[ 'members' ] as $members ) {
							if ( $members[ 'character' ][ 'level' ] == '110' && $members[ 'character' ][ 'name' ] != $character ) {
				...
				echo 'Character does not need to be updated (logout timestamp is identical with database).';
			}
		...
		echo 'The server could not retrieve the data of ' . $character . ' @ ' . $region . '-' . $realm . '.<br />This can have several reasons:<br />– incorrect information<br />– character too low level (only accepting level 110)<br />– character transfered<br />– Blizzard API unavailable.';
	}
	...
	$delete = mysqli_query( $stream, "DELETE FROM `000_import` WHERE `c` = '" . $character . "' AND `r` = '" . $region . "' AND `s` = '" . $realm . "'" );
...
echo '<meta http-equiv="refresh" content="1;url=http://apinfo.gerritalex.de/var/large_importer.php" />';
