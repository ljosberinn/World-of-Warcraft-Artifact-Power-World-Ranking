...
if ( isset( $_GET[ 'character' ] ) && isset( $_GET[ 'region' ] ) && strlen( $_GET[ 'region' ] ) == '2' && isset( $_GET[ 'realm' ] ) ) {

	$key = '';

	$character = ucfirst( strtolower( $_GET[ 'character' ] ) );
	$region = strtoupper( $_GET[ 'region' ] );
	$realm = ucfirst( strtolower( $_GET[ 'realm' ] ) );	
	...
	if ( $data != '' ) {
		...
		if ( $data[ 'level' ] == '110' ) {
			...
			<table>
				...
				<tbody>
					<tr>
						<td class="' . $cell_class . '"><a href="http://armory.gerritalex.de/?r=' . $region . '&s=' . ucfirst( strtolower( $_GET[ 'realm' ] ) ) . '&c=' . $character . '">' . $character . '</a><br /><a class="region" onclick="left_table_region_filter(this.innerText);">' . $region . '</a> <a class="realm" onclick="left_table_realm_filter(' .$realm. ');">' . $_GET[ 'realm' ] . '</a></td>
						<td data-sort-value="' .$artifact_power. '" style="background-color: #84724E;">' . number_format( ( $artifact_power / 1000000000 ), 3 ) . ' B</td>
						<td data-sort-value="' .$artifact_level. '" style="background-color: #84724E;">' . $artifact_level . '</td>
						<td data-sort-value="' .$ilvlaverage. '" style="background-color: #84724E;">' . $ilvlaverage . '/' . $ilvlaveragebags . '</td>
						<td data-sort-value="' .$sum. '" style="background-color: #84724E;">' . $sum . '</td>
						<td data-sort-value="' .($raid1+$raid2+$raid3+$raid4). '" style="background-color: #84724E;">' . $raid1 . ' ' . $raid2 . ' ' . $raid3 . ' ' . $raid4 . '</td>
						<td data-sort-value="' .$eq. '" style="background-color: #84724E;">' . round( $eq ) . '</td>
					<tr>
				</tbody>
			</table>';
			
			// REMOVE FROM QUEUE IF ENTRY EXISTS
			$query = mysqli_fetch_array(mysqli_query($stream, "SELECT `id` FROM `000_import` WHERE `c` = '" .$character. "' AND `r` = '" .$region. "' AND `s` = '" .ucfirst( strtolower( $_GET[ 'realm' ] ) ). "'"));
			if($query['id'] != '') {
			...
			if ( $data[ 'guild' ][ 'name' ] != '' ) {
				...
				$realm = mysqli_fetch_array( mysqli_query( $stream, "SELECT `short` FROM `000_ovw_realms` WHERE `name` = '" . ucfirst( strtolower( $_GET[ 'realm' ] ) ) . "'" ) );
				$realm = $realm[ 'short' ];
			...
			echo 'The character in question is not level 110.';
		}

	} else {
		echo '<table><tbody><tr><td class="trash">The server could not retrieve the data of ' . $character . ' @ ' . $region . '-' . $realm . '.<br />This can have several reasons:<br />– incorrect information<br />– character too low level (only accepting level 110)<br />– character transfered<br />– Blizzard API unavailable.</td></tr></tbody></table>';
	}
}

else {
	echo 'To use this file, please define the following GET-parameters:<br />
	character = character name<br />
	region = region (EU, TW, KR, US)<br />
	realm = realm name in native language (e.g. 暗影之月 for taiwanese server Shadowmoon)';
}
