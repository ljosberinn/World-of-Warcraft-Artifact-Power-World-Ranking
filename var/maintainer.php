...
if ( isset( $_GET[ 'class' ] ) && is_numeric( $_GET[ 'class' ] ) && $_GET[ 'class' ] <= '12' && $_GET[ 'class' ] >= 1 ) {
	...
	$threshold = time( 'now' ) - 43200;
	
	
	
	$character_fetcher = mysqli_fetch_array( mysqli_query( $stream, "SELECT `name`, `region`, `realm`  FROM `" . $global_class_table . "` WHERE `update` <= '" . $threshold . "' LIMIT 1" ) );	
		

	$character = $character_fetcher[ 'name' ];
	$region = $character_fetcher[ 'region' ];
	$realm = $character_fetcher[ 'realm' ];

	include( 'stream.php' );

	$realm = mysqli_fetch_array( mysqli_query( $stream, "SELECT `name`, `short` FROM `000_ovw_realms` WHERE `id` = '" . $realm . "'" ) );
	$realm = $realm[ 'short' ];
	$real_name = $realm[ 'name' ];
	...
	if ( $data != '' ) {
		...
		if ( $data[ 'level' ] == '110' ) {
			...
			$table_name = '' . $region . '_' . $character_fetcher[ 'realm' ] . '';
			// CHECK IF ALREADY EXISTS IN REALM TABLE
			$query = mysqli_fetch_array( mysqli_query( $stream, "SELECT `id`, `logout` FROM `" . $table_name . "` WHERE `name` = '" . $character . "'" ) );
			...
			if ( $query[ 'logout' ] != $logout ) {
				...
				$query = mysqli_fetch_array( mysqli_query( $stream, "SELECT `id` FROM `000_import` WHERE `c` = '" . $character . "' AND `r` = '" . $region . "' AND `s` = '" . ucfirst( strtolower( $real_name ) ) . "'" ) );
				if ( $query[ 'id' ] != '' ) {
...
if ( $character == $_GET[ 'character' ] && $_GET['character'] != '' ) {
	...
	if ( $_GET[ 'counter' ] == 300 ) {
		...
		$delete_realm_entry = mysqli_query( $stream, "DELETE FROM `" . $table_name . "` WHERE `name` = '" . $_GET[ 'character' ] . "'" );
		$delete_top1000_entry = mysqli_query( $stream, "DELETE FROM `" . $global_class_table . "` WHERE `name` = '" . $_GET[ 'character' ] . "' AND `realm` = '" . $character_fetcher[ 'realm' ] . "'" );
		$delete_global_entry = mysqli_query( $stream, "DELETE FROM `000_top1000_global` WHERE `name` = '" . $_GET[ 'character' ] . "' AND `realm` = '" . $character_fetcher[ 'realm' ] . "'" );

		if ( $delete_global_entry && $delete_realm_entry && $delete_top1000_entry ) {
			$counter = 0;
		}
	}

	echo '<meta http-equiv="refresh" content="2;url=http://apinfo.gerritalex.de/var/maintainer.php?class=' . $_GET[ 'class' ] . '&character=' . $character . '&counter=' . $counter . '" />';
} else {
	echo '<meta http-equiv="refresh" content="2;url=http://apinfo.gerritalex.de/var/maintainer.php?class=' . $_GET[ 'class' ] . '&character=' . $character . '" />';
}
