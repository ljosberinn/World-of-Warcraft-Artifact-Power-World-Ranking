...
if ( isset( $_GET[ 'entry' ] ) && is_numeric( $_GET[ 'entry' ] ) && isset( $_GET[ 'realm' ] ) && is_numeric( $_GET[ 'realm' ] ) ) {

	include( 'stream.php' );

	$region = mysqli_fetch_array( mysqli_query( $stream, "SELECT `short`, `region` FROM `000_ovw_realms` WHERE `id` = '" . $_GET[ 'realm' ] . "'" ) );
	$realm = $region[ 'short' ];
	$region = $region[ 'region' ];

	$table = '' . $region . '_' . $_GET[ 'realm' ] . '';

	$information = mysqli_fetch_array( mysqli_query( $stream, "SELECT `name` FROM `" . $table . "` WHERE `id` = '" . $_GET[ 'entry' ] . "'" ) );
