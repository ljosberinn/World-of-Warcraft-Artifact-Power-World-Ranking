...
if ( isset( $_GET[ 'region' ] ) && strlen( $_GET[ 'region' ] ) == '2' && ( $_GET[ 'region' ] == 'EU' ) || ( $_GET[ 'region' ] == 'US' ) || ( $_GET[ 'region' ] == 'TW' ) || ( $_GET[ 'region' ] == 'KR' ) || ( $_GET[ 'region' ] == 'global' ) ) {
	...
	$regions = array( 'EU', 'US', 'KR', 'TW', 'global' );

	$selector = '<select onchange="left_table_region_filter(this.value);">';

	foreach ( $regions as $region ) {
		if ( $region == $_GET[ 'region' ] ) {
			$selector .= '<option disabled selected>' . $_GET[ 'region' ] . '</option>';
		} else {
			$selector .= '<option value="' . $region . '">' . $region . '</option>';
		}
	}
	...
	if ( $_GET[ 'region' ] != 'global' ) {
		$global_table = mysqli_query( $stream, "SELECT * FROM `000_top1000_global` WHERE `region` = '" . $_GET[ 'region' ] . "' ORDER BY `ap` DESC LIMIT 50" );
	} else {
